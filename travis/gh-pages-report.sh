...
if [ "$TRAVIS_BRANCH" == "master" ]; then
    ...
    git checkout -b new_master --track old/master
    cd travis/loadreport    
    ls -al
    #sleep 1m
    phantomjs loadreport.js ${TEST_PAGE} filmstrip
    phantomjs speedreport.js ${TEST_PAGE} $LATEST_SHA
    git add -f reports/.
    echo "*** Contents of 'travis/loadreport/reports':"
    ls -al reports
    git commit -m "Travis build $TRAVIS_BUILD_NUMBER" # pushed to gh-pages"
    git push https://${GH_TOKEN}@github.com/${REPO} new_master > /dev/null #gh-pages > /dev/null
fi